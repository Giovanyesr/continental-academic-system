...
export default function Sidebar() {
  ...
  return (
    <aside className="hidden lg:flex fixed left-0 top-0 h-screen w-[280px] bg-gradient-to-br from-[#2D0B5A] via-[#1A0538] to-[#120227] flex-col z-50 border-r border-[#6802C1]/30">
      
      {/* Header con Logo y User Info Combinados estilo Premium */}
      <div className="p-6">
        <Link href="/" className="flex items-center gap-4 group mb-8">
          <div className="w-12 h-12 bg-gradient-to-tr from-[#FFC20E] to-[#FFF1C1] rounded-xl flex items-center justify-center shadow-lg shadow-[#FFC20E]/20 transform group-hover:scale-105 transition-transform p-2">
            <Image src="/logocontinental.png" alt="UC" width={40} height={40} className="object-contain" />
          </div>
          <div>
            <p className="text-[10px] font-bold text-[#FFC20E] tracking-[0.2em] uppercase">Universidad</p>
            <p className="text-xl font-black text-white tracking-tight">Continental</p>
          </div>
        </Link>
        ...
        {estudiante && (
          <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center gap-4 transition-all hover:bg-white/10">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-white truncate">{estudiante.nombre.split(' ').slice(0, 2).join(' ')}</p>
              <p className="text-[11px] font-medium text-[#FFC20E] truncate mt-0.5">{estudiante.carrera}</p>
      ...
      <nav className="flex-1 px-4 overflow-y-auto custom-scrollbar pb-6 space-y-1">
        <p className="px-4 text-[11px] font-bold text-white/40 uppercase tracking-widest mb-3 mt-2">Principal</p>
      ...
      <div className="p-5 border-t border-white/10 bg-black/10">
        {estudiante ? (
          ...
          <Link
            href="/login"
            className="flex items-center justify-center gap-2 py-3.5 bg-gradient-to-r from-[#FFC20E] to-[#EAB308] text-[#1A0538] font-bold rounded-xl transition-all hover:scale-[1.02] shadow-lg shadow-[#FFC20E]/20"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
